...
# install tools
RUN apk add --no-cache --update curl tar bash jq git
...
# instal ffmpeg based on https://github.com/jrottenberg/ffmpeg/blob/master/docker-images/3.4/alpine/Dockerfile
...
# create symlinks to ffmpeg & ffprobe
RUN ln -s /usr/local/bin/ffmpeg /usr/bin/ffmpeg && \
    ln -s /usr/local/bin/ffprobe /usr/bin/ffprobe

WORKDIR /

# install OpenJDK8 based on https://github.com/docker-library/openjdk/blob/master/8-jdk/alpine/Dockerfile
...
# install Maven based on https://github.com/carlossg/docker-maven/blob/master/jdk-8-alpine/Dockerfile
